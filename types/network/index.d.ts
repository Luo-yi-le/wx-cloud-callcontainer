...
declare module '../../index' {
    namespace request {
        ...
        interface Option<U extends string | WeChatGeneral.IAnyObject | ArrayBuffer = any | any> {
            /** 开发者服务器接口地址 */
            url: string
            /** 接口调用结束的回调函数（调用成功、失败都会执行） */
            complete?: (res: WeChatGeneral.CallbackResult) => void
            /** 请求的参数 */
            data?: U
            /** 返回的数据格式 */
        ...
        interface SuccessCallbackResult<T extends string | WeChatGeneral.IAnyObject | ArrayBuffer = any | any> extends WeChatGeneral.CallbackResult {
            /** 开发者服务器返回的数据 */
            data: T
   ...
   *
   * ```tsx
   * const requestTask = WeChat.request({
   ...
   *
   * ```tsx
   * const requestTask = WeChat.request(params)
   ...
   */
    interface RequestTask<T> extends Promise<request.SuccessCallbackResult<T>> {
        ...
        offHeadersReceived(
            ...
            callback: (res: WeChatGeneral.CallbackResult) => void,
        ): void
    ...
    interface WeChatStatic {
         ...
         *
         * ```tsx
         * const res = await WeChat.request(params)
         * ```
         ...
         *
         * 在调用 `WeChat.request` 发起请求之前，调用 `WeChat.addInterceptor` 方法为请求添加拦截器，拦截器的调用顺序遵循洋葱模型。
         * 拦截器是一个函数，接受 chain 对象作为参数。chain 对象中含有 **requestParmas** 属性，代表请求参数。拦截器内最后需要调用 `chain.proceed(requestParams)` 以调用下一个拦截器或发起请求。
         *
         * WeChat 提供了两个内置拦截器 `logInterceptor` 与 `timeoutInterceptor`，分别用于打印请求的相关信息和在请求超时时抛出错误。
         ...
         *     })
         *   }
         * WeChat.addInterceptor(interceptor)
         * WeChat.request({ url })
         * ```
         * @example
         * ```tsx
         * WeChat.addInterceptor(WeChat.interceptors.logInterceptor)
         * WeChat.addInterceptor(WeChat.interceptors.timeoutInterceptor)
         * WeChat.request({ url })
         * ```
         */
